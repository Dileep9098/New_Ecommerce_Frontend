...
export default function SubscriberDashboard() {
    ...
    useEffect(() => {
        ...
        const fatchData = async () => {
            try {
                //   debugger
                const response = await axiosInstance.get(Config.END_POINT_LIST["GET_ALL_SUBSCRIBERS"], { withCredentials: true })
                // console.log(response)

                if (response.data.success) {
                    setAllSubscribers(response.data.subscribersList)
                }
                else {
    ...
    const handleDelete = async (id) => {
        try {
            //   debugger
            const response = await axiosInstance.delete(`${Config.END_POINT_LIST["DELETE_SUBSCRIBERS"]}/${id}`, { withCredentials: true })
            // console.log(response)

            if (response.data.success) {
                showSuccessMsg(response.data.message)
                setAllSubscribers((prevState) => prevState.filter((contactDetails) => contactDetails._id !== id));
            }
            else {
                showErrorMsg(response.data.message)
            }
    ...
    const filteredContactList = (allSubscribers || []).filter(contactDetails =>
        contactDetails.email.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const totalPages = Math.ceil(filteredContactList.length / itemsPerPage);
         ...
         const handleExport = () => {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Subscribe Details");
            XLSX.writeFile(wb, "Subscribe_details.xlsx");
        };
                        ...
                        <div className="page-title-box">
                            <div className="page-title-right">
                                <ol className="breadcrumb m-0">
                                    <li className="breadcrumb-item"><Link to="#" className='text-primary'> Subscribers List</Link></li>
                                    <li className="breadcrumb-item active">Subscribers List</li>
                                </ol>
                            </div>
                            <h4 className="page-title"><i className="ri-align-left me-2"></i>Subscribers List</h4>
                        </div>
                    </div>
                </div>
